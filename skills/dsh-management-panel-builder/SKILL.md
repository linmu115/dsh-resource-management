...
description: 为现有 DSH Plugin 或 Skill 设计、实现并验证 dsh-resource-management 参数面板，包括 panel.yaml、DSH Settings 真实配置桥、模型目录选择和操作按钮。用于把指定资源适配到 Manager 的统一参数设置框架；不用于重做 Manager UI、Git/Generation 管理或凭空增加目标资源没有的功能。
---
...
在编写声明前必须阅读 [Contract v2、DSH Settings 桥与方案 A 模板](references/panel-contract-v2.md)。需要创建文件时，从 [panel.yaml 模板](assets/dsh-management/panel.yaml) 复制并删改不适用的示例，不要让占位字段进入发布产物。

## 真实 Cordis 配置桥是硬门槛

先检查目标 Plugin 是否已用 `@deepseek-ai/dsh-settings` 注册自己拥有的 Settings namespace。只要字段代表 Cordis 配置，默认实施顺序必须是：

1. 目标 Plugin 定义并注册 namespace Schema，以 Cordis entry config 为 `base`；
2. 目标 Plugin 从该 namespace 的解析值读取真实运行配置；
3. `panel.yaml` 用 `binding.kind: dsh-settings` 指向同一 namespace 和字段路径；
4. Manager 通过官方 `ctx.settings.describe()` / `mutate()` 读写用户覆盖层；
5. 测试证明保存后真实消费者改变行为，冷重启后仍保留。

不得让目标资源读取 Manager 私有 `panel-values.json`，不得让 AI 只写 YAML 后声称配置已经生效，也不得直接修改已安装包中的静态 `cordis.patch.yml`。只有目标资源的数据不属于 Cordis/DSH Settings 时，才考虑文档化的 `plugin-data` Adapter。
...
4. 每个字段在接受声明前都必须列出 Cordis 配置字段、Settings namespace/key、运行消费者、持久化位置和生效模式。仅看到 Manager 控件、仅写入 Manager 的 `panel-values.json`，不能证明目标插件已经使用该值。纯 action 面板不需要字段配置桥，但每个 action 仍必须登记真实处理器。
...
9. 模型配置使用 Contract v2 `model-select`，从 DSH 当前模型目录选择 provider/model/reasoning effort；禁止用自由文本框假装模型目录。允许继承时以 `null` 表示继承，并由目标 Plugin 明确定义继承来源。
10. 仅开发者需要的低层配置放进 `collapsible: true`、`defaultCollapsed: true` 的分组，并解释风险；不要把内部顺序、调试开关混入普通设置。
...
- 对应 Contract 版本严格解析通过，目标安装包确实包含 `dsh-management/panel.yaml`。
- README 与参数设置 Tab 均可用；无效面板不会破坏 README 或其他资源。
- 每种控件验证默认值、保存、恢复默认、校验错误和正确的生效时机。
- 目标插件通过运行行为测试证明参数生效；操作按钮验证登记、并发保护、确认和错误路径。
- `dsh-settings` 字段验证 namespace 已注册、revision 冲突安全、恢复默认重新继承 Cordis base，且没有写入 Manager 私有 JSON。
