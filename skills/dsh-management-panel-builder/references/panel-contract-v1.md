...
如果 action 会停止、重启当前 DSH，或以其他方式中断承载 Manager HTTP 请求的进程，必须先用 `ctx.resourceManagementActions.deferUntilResponse(task)` 登记实际操作，再立即返回确认结果。Manager 只会在完整 JSON 回执已经写给浏览器后执行该 task，避免浏览器收到空响应。不得只依赖固定延时猜测响应何时完成。
